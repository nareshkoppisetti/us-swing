"""Agent 36 — Gold & Precious Metals | Commodity | Tier 1"""
import time, logging, numpy as np
from datetime import datetime
from modules.agents.base_agent import BaseAgent, AgentOutput, Signal
logger = logging.getLogger("agents")

class Agent36(BaseAgent):
    agent_id=36; agent_name="Gold & Precious Metals"; category="Commodity"
    ...
    def run(self, symbol, context):
        ...
        try:
            ...
            df = svc.get_ohlcv("GC=F", period="6mo")
            if df is None or len(df) < 20: raise ValueError("No Gold data")
            ...
            bf = [f"Gold above 20-day MA — bullish"] if px>sma20 else []
            brf= [f"Gold below 20-day MA — bearish"] if px<sma20 else []
            if rsi > 70: brf.append(f"RSI={rsi:.1f} — Gold overbought")
            ...
            return AgentOutput(agent_id=self.agent_id, agent_name=self.agent_name,
                ...
                weight=0.7, reasoning=f"Gold={px:.2f}, SMA20={sma20:.2f}, RSI={rsi:.1f}, 1m={ret_1m:.1f}%",
                ...
                llm_ready_summary={"agent":self.agent_name,"signal":signal.value,
                    "finding":f"Gold={px:.2f}; 1m={ret_1m:.1f}%; RSI={rsi:.1f}"},
