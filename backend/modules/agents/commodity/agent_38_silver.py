"""Agent 38 — Silver Intelligence | Commodity | Tier 1"""
...
class Agent38(BaseAgent):
    agent_id=38; agent_name="Silver Intelligence"; category="Commodity"
    ...
    def run(self, symbol, context):
        ...
        try:
            ...
            df = svc.get_ohlcv("SI=F", period="6mo")
            if df is None or len(df) < 20: raise ValueError("No Silver data")
            ...
            bf = [f"Silver above 20-day MA — bullish"] if px>sma20 else []
            brf= [f"Silver below 20-day MA — bearish"] if px<sma20 else []
            if rsi > 70: brf.append(f"RSI={rsi:.1f} — Silver overbought")
            ...
            return AgentOutput(agent_id=self.agent_id, agent_name=self.agent_name,
                ...
                weight=0.7, reasoning=f"Silver={px:.2f}, SMA20={sma20:.2f}, RSI={rsi:.1f}, 1m={ret_1m:.1f}%",
                ...
                llm_ready_summary={"agent":self.agent_name,"signal":signal.value,
                    "finding":f"Silver={px:.2f}; 1m={ret_1m:.1f}%; RSI={rsi:.1f}"},
