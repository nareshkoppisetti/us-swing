"""Agent 39 — Copper Intelligence | Commodity | Tier 1"""
import time, logging, numpy as np
from datetime import datetime
from modules.agents.base_agent import BaseAgent, AgentOutput, Signal
logger = logging.getLogger("agents")

class Agent39(BaseAgent):
    agent_id=39; agent_name="Copper Intelligence"; category="Commodity"
    ...
    def run(self, symbol, context):
        ...
        try:
            ...
            df = svc.get_ohlcv("HG=F", period="6mo")
            if df is None or len(df) < 20: raise ValueError("No Copper data")
            ...
            bf = [f"Copper above 20-day MA — bullish"] if px>sma20 else []
            brf= [f"Copper below 20-day MA — bearish"] if px<sma20 else []
            if rsi > 70: brf.append(f"RSI={rsi:.1f} — Copper overbought")
            ...
            return AgentOutput(agent_id=self.agent_id, agent_name=self.agent_name,
                ...
                weight=0.7, reasoning=f"Copper={px:.2f}, SMA20={sma20:.2f}, RSI={rsi:.1f}, 1m={ret_1m:.1f}%",
                ...
                llm_ready_summary={"agent":self.agent_name,"signal":signal.value,
                    "finding":f"Copper={px:.2f}; 1m={ret_1m:.1f}%; RSI={rsi:.1f}"},
