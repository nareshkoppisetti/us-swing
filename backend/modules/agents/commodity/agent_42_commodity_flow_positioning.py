"""Agent 42 — Commodity Flow Positioning | Commodity | Tier 1 | depends: [20, 40]"""
...
class Agent42(BaseAgent):
    agent_id=42; agent_name="Commodity Flow Positioning"; category="Commodity"
    refresh_frequency="daily"; dependencies=[20, 40]; tier=1

    def run(self, symbol, context):
        start = time.time()
        try:
            dep_scores = [context[d].score for d in [20, 40] if d in context and context[d] and not context[d].error]
            ...
            return AgentOutput(agent_id=self.agent_id, agent_name=self.agent_name,
                ...
                weight=0.65, reasoning=f"Commodity Flow Positioning: composite from {len(dep_scores)} upstream agents. Score={score:.1f}",
